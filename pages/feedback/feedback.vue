<template>
	<view>
		...
		<view class="feedback-content">
			<textarea class="feedback-textarea" v-model="content" placeholder="请输入您想要反馈的问题" />
			</view>
		<view class="feedback-image">反馈图片:</view>
	...
	export default {
		...
		methods: {
			...
			async uploadFiles(filePath) {
				const cloudPath = (Math.random()*10000000).toString(16).substr(0,4)+'-'+(new Date()).getTime()+'-'+Math.random().toString().substr(2,5)
			...
			updateFeedback: function({content, feedbackImages}) {
